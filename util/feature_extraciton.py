...
:purpose: This module contains supporting methods for extracting features from
          SeisBench prediction traces to inform picks for subsequent probabalistic
          analyses (e.g., NonLinLoc)
"""
...
def process_predictions(prediction_trace, et_obs=None, thr_on=0.1, thr_off=0.1, fit_pad_sec=0.1, fit_thr_coef=0.1, ndata_bounds=[30, 9e99]):
    """
    Extract statistical fits of normal distributions to prediction peaks from ML prediction traces that trigger
    ...
    :param fit_pad_sec          [float] amount of padding on either side of data bounded by trigger ON/OFF times
                                    for calculating Gaussian fits to the probability peak(s)
    :param fit_thr_coef:        [float] Gaussian fit data 
    :param ndata_bounds         [2-tuple of int] minimum & maximum count of data for each trigger window

    :: OUTPUT ::
    :return df_out:             [pandas.dataframe.DataFrame]
                                    DataFrame containing the following metrics for each trigger and observed pick
                                    ...
                                    'det_obs_prob' - delta time [seconds] of observed pick time minus et_max
                                                        Note: this will be np.nan if there are no picks in the trigger window
    ...
    df_out = pd.DataFrame(feature_holder, columns=cols)
    return df_out     
...
# def get_trigger_windows(pred_values, thr_on= 0.1, thr_off = 0.1, min_len=30, max_len=9e99):
#     """
#     Modified directly from source-code of obspy.trigger.trigger_onset()
#     method to include a minimum trigger length option and enforces a mandatory
#     drop of triggers that fall outside min_len <= trigger_len <= max_len.

#     :: INPUTS ::
#     :param pred_values: [numpy.ndarray] prediction values 
#     :param thr_on: [float] threshold value for turning on trigger
#     :param thr_off: [float] threshold value for turning off trigger
#     :param       
    
#     """
#     # Create threshold filter - based on obspy.stream.trigger
#     ind1 = np.where(pred_values >= thresholds[0])[0]
#     ind2 = np.where(pred_values >= thresholds[1])[0]
#     # Generate Double Ended QUEues (deque)
#     on = deque([ind1[0]])
#     of = deque([-1])
#     # Determine indices where the CRF falls below threshold
#     ind2_ = np.empty_like(ind2, dtype=bool)
#     ind2_[:-1] = np.diff(ind2) > 1
#     # Last occurrence is missed by diff, so add it manually
#     ind2_[-1] = True
#     of.extend(ind2[ind2_].tolist())
#     on.extend(ind1[np.where(np.diff(ind1) > 1)[0] + 1].tolist())

#     # drop last pick if trigger is on
#     of.extend([1e99])
#     on.extend([on[-1]])

#     # add last sample to ensure trigger gets switched off if ctf does not fall
#     # below off-threshold before hitting the end
#     of.append(len(pred_values))
#     # Create list of pick on and off times in samples
#     pick = []
#     while on[-1] > of[0]:
#         while on[0] <= of[0]:
#             on.popleft()
...
#         elif of[0] - on[0] < min_len:
#             on.popleft()
#             of.appendleft(on[0] + min_len)
